...
#the filtered neighborhoods will be written to a file after the blast db's etc 
#(so order files by time and feed the latest ones into clinker) 
...
#(i.e. the largest record from a given taxid may not be the one that is kept).
...
#Multi-query input.  format - ("path/to/folder/with/cblaster_binary_file.txt", integer_bgc_length)
folders = [None, None]
...
ement.  no requirmen for coverage in case the annots are broken